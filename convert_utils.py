...
OPENAI_KEY = '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'
...
def whisper(audio_path):
    response = openai.Audio.transcribe("whisper-1", audio_path, language="ja")
...
def send_chatgpt(message):
    gpt = OpenAI(model_name="gpt-3.5-turbo",max_tokens=512,temperature=1,frequency_penalty=0.02,openai_api_key=OPENAI_KEY,)
    result = gpt(message)
    return result
